...
import { handleAddSkillSubmit } from "./serverAction";
...
export default function AddSkill({ aboutId }: { aboutId: string }) {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      setIsLoading(true);
      const response = await handleAddSkillSubmit(aboutId, formState);
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="px-2 border border-box-color dark:bg-box-color rounded-md dark:text-white text-box-color placeholder:text-sm placeholder:text-slate-500 focus:outline-none focus:border-blue-500 disabled:text-slate-500 disabled:border-slate-500"
      />
      {(formState?.title || formState?.skill) && (
        <button
          type="submit"
          disabled={isLoading}
